    ...
    if (showMap && mapRef.current) {
      ...
      users.forEach((user) => {
        ...
        const url = `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(
          user.location
        )}&key=AIzaSyCVkP1_uzFeBJ4Jay9lip_g6A6XDbrm7bY`;
  ...
  return (
    <>
      <div className="py-4 px-0.2 sm:px-8 grid grid-cols-2 gap-2 md:grid-cols-3 sm:gap-4">
        <h2 className="text-2xl font-bold sm:mb-4 col-span-3">
        ...
        <div className="col-span-3 ">
          ...
          <button
            ...
            className="ml-4 bg-green-400 hover:bg-green-700 text-white font-bold py-2 px-4 rounded"
          >
      ...
      <div className="py-4 px-0.2 sm:px-8 grid grid-cols-2 gap-2 md:grid-cols-3 sm:gap-4">
        {users.map((user) => (
          <Link key={user.id} to={`/profile/${user.uid}`}>
            <div
              key={user.id}
              className="bg-white  w-full h-full p-2 rounded-md shadow-md items-center inline-flex min-w-min hover:translate-y-1 duration-500 ease-in-out"
            >
              {" "}
              <img
                src={user.image || avatar}
                alt="userImage"
                className="w-10 h-10 mr-2 sm:mr-3 rounded-full"
              />
              <div>
      ...
      {showMap && <div className="col-span-3 h-96 mt-4" ref={mapRef}></div>}
    </>
  );
};
